...
- Wrote complete sprint backlog to `.squad/backlog.md` — 38 issues across 5 sprints
- Each sprint is one npm release: v0.1.0 (parser+index) through v0.5.0 (MCP+Obsidian)
- Sprint gates assigned to Flynn (review+approve before merge), release notes to Dumont
- Scope documented in `.squad/decisions/inbox/castor-backlog-scope.md`
- Key decisions: Sprints 1-3 are core (locked in), Sprints 4-5 are extensions (can be adapted)
...
- Backlog enables parallelization and prevents mid-sprint scope creep
- Coverage target 80% minimum, parser/governance/indexer >= 95%

### 2026-04-03 — Sprint 2 backlog refined with TypeScript signatures
- Updated all Sprint 2 implementation issues with explicit TypeScript function signatures
- Added type definitions and exports for query/graph modules to prevent Sprint 1 API mismatch
...
- Sprint 2 gate updated: added ACs for public API exports and test skeleton timing (after types are approved)
- Decision documented in `.squad/decisions/inbox/castor-phase2-backlog-refined.md`
- Prevention: Test skeletons now written after types locked, eliminating Yori↔Tron contract drift from Sprint 1

### 2026-04-03 — Sprint 3 backlog refinement and Wave 0 kickoff plan

**Applied Sprint 2 Retro Action Items (A1-A8) to Sprint 3:**
...
- **A6**: Sprint 3 gate checklist written at Wave 0 kickoff (→ `.squad/decisions/inbox/castor-phase3-gate.md`)
...
**Sprint 3 Wave Plan (4 waves, 7 days total):**
...
- 16 Sprint 3 specific:
  ...
  - **#22**: Indexer coverage improved to ≥ 90% (Sprint 2 debt)

**New TypeScript Types (Sprint 3 additions to types.ts):**
...
- `SearchResult` (future Sprint 4, documented but not implemented yet)
...
**Prevention Measures (from Sprint 2 Retro):**
...
- `.squad/backlog.md`: Sprint 3 Refinement section (line ~499), Wave 0-4 plans (line ~548-690), refined ACs for all 8 Sprint 3 issues (line ~702-1046)
