...
describe('parseFile', () => {
  ...
  describe('frontmatter extraction', () => {
    ...
    it('returns the correct filename stem', async () => {
      const result = await parseFile(join(BASIC_VAULT, 'overview.md'))
      expect(result.filename).toBe('overview')
    })
    ...
    it('handles empty file — returns valid ParsedFile with empty collections', async () => {
      ...
      expect(result.wikilinks.size).toBe(0)
      expect(result.typedRelations.size).toBe(0)
  ...
  describe('tag extraction', () => {
    it('finds simple tags in body', async () => {
      ...
      expect(result.tags.has('project/beta')).toBe(true)
      expect(result.tags.has('status/pending')).toBe(true)
    })
    ...
    it('expands hierarchical tags to all ancestor levels', async () => {
      ...
      const result = await parseFile(join(BASIC_VAULT, 'overview.md'))
      expect(result.tags.has('project')).toBe(true)
      expect(result.tags.has('project/alpha')).toBe(true)
      expect(result.tags.has('project/alpha/planning')).toBe(true)
      expect(result.tags.has('status')).toBe(true)
      expect(result.tags.has('status/active')).toBe(true)
    })
  ...
  describe('wikilink extraction', () => {
    it('finds [[wikilinks]] in body', async () => {
      ...
      const result = await parseFile(join(BASIC_VAULT, 'overview.md'))
      expect(result.wikilinks.has('note-two')).toBe(true)
      expect(result.wikilinks.has('prerequisite')).toBe(true)
    })
    ...
    it('normalizes wikilinks to lowercase', async () => {
      ...
      const result = await parseFile(join(BASIC_VAULT, 'overview.md'))
      expect(result.wikilinks.has('note-one')).toBe(true)
      // The uppercase variant must NOT be stored separately
      expect(result.wikilinks.has('NOTE-ONE')).toBe(false)
    })
  ...
  describe('typed relations', () => {
    it('identifies frontmatter keys with [[wikilink]] values as typed relations', async () => {
      ...
      expect(result.typedRelations.has('depends_on')).toBe(true)
      expect(result.typedRelations.has('blocks')).toBe(true)
      expect(result.typedRelations.get('depends_on')).toContain('prerequisite')
      expect(result.typedRelations.get('blocks')).toContain('note-one')
    })
    ...
    it('supports multiple targets per relation type', async () => {
      ...
      const targets = result.typedRelations.get('related_to') ?? []
